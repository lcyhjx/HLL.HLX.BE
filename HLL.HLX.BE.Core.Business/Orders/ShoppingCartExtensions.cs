    ...
    public static class ShoppingCartExtensions
    {
            ...
            out int cycleLength, out RecurringProductCyclePeriod cyclePeriod, out int totalCycles)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    throw new UserFriendlyException(string.Format("Product (Id={0}) cannot be loaded", sci.ProductId));
                }

                if (product.IsRecurring)
                {
                    string conflictError = InfoMsg.ShoppingCart_ConflictingShipmentSchedules;                        
        ...
        public static User GetCustomer(this IList<ShoppingCartItem> shoppingCart)
        {
