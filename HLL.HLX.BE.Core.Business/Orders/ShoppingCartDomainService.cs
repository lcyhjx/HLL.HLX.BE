...
namespace HLL.HLX.BE.Core.Business.Orders
{
    ...
    /// </summary>
    public class ShoppingCartDomainService : DomainService
    {
        #region Fields

        private readonly IShoppingCartItemRepository _sciRepository;
        ...
        private readonly CheckoutAttributeDomainService _checkoutAttributeService;
        private readonly ICheckoutAttributeParser _checkoutAttributeParser;
        private readonly IPriceFormatter _priceFormatter;
        private readonly UserDomainService _customerService;
        ...
        public ShoppingCartDomainService(IShoppingCartItemRepository sciRepository,
            ...
            UserDomainService customerService,
        ...
        {
            var query = from sci in _sciRepository.GetAll()
        ...
        public virtual IList<string> GetRequiredProductWarnings(User customer,
                        ...
                        if (product.AutomaticallyAddRequiredProducts)
                        {
                            ...
                            if (automaticallyAddRequiredProductsIfEnabled)
                            {
                                ...
                                if (addToCartWarnings.Count > 0)
                                {
                                    ...
                                    warnings.Add(string.Format(InfoMsg.ShoppingCart_RequiredProductWarning, rp.Name));
                                }
                            }
                            else
                            {
        ...
        /// <returns>Warnings</returns>
        public virtual IList<string> GetStandardWarnings(User customer, ShoppingCartType shoppingCartType,
            ...
            int quantity)
        {
            ...
            if (product.Deleted)
            {
                warnings.Add(InfoMsg.ShoppingCart_ProductDeleted);
                return warnings;
            }
            ...
            {
                warnings.Add(InfoMsg.ShoppingCart_ProductUnpublished);
            }
            ...
            {
                warnings.Add(string.Format(InfoMsg.ShoppingCart_MinimumQuantity, product.OrderMinimumQuantity));
                hasQtyWarnings = true;
            }
            if (quantity > product.OrderMaximumQuantity)
            {
                warnings.Add(string.Format(InfoMsg.ShoppingCart_MaximumQuantity, product.OrderMaximumQuantity));
                hasQtyWarnings = true;
            }
            var allowedQuantities = product.ParseAllowedQuantities();
            if (allowedQuantities.Length > 0 && !allowedQuantities.Contains(quantity))
            {
                warnings.Add(string.Format(InfoMsg.ShoppingCart_AllowedQuantities, string.Join(", ", allowedQuantities)));
            }

            var validateOutOfStock = shoppingCartType == ShoppingCartType.ShoppingCart || ! ShoppingCartSettings.AllowOutOfStockItemsToBeAddedToWishlist;
                            ...
                            if (product.BackorderMode == BackorderMode.NoBackorders)
                            {
                                ...
                                if (maximumQuantityCanBeAdded < quantity)
                                {
                                    if (maximumQuantityCanBeAdded <= 0)
                                        warnings.Add(InfoMsg.ShoppingCart_OutOfStock);
                                    else
                                        warnings.Add(string.Format(InfoMsg.ShoppingCart_QuantityExceedsStock, maximumQuantityCanBeAdded));
                                }
                            }
                        }
                        break;
            ...
            if (product.AvailableStartDateTimeUtc.HasValue)
            {
                ...
                {
                    warnings.Add(InfoMsg.ShoppingCart_NotAvailable);
        ...
        /// <returns>Warnings</returns>
        public virtual IList<string> GetShoppingCartItemAttributeWarnings(User customer,
            ShoppingCartType shoppingCartType,
            Product product,
            int quantity = 1,
            ...
            bool ignoreNonCombinableAttributes = false)
        {
            ...
            foreach (var a2 in attributes2)
            {
                if (a2.IsRequired)
                {
                    ...
                    if (!found)
                    {
                        var notFoundWarning = !string.IsNullOrEmpty(a2.TextPrompt) ?
                            a2.TextPrompt :
                            string.Format(InfoMsg.ShoppingCart_SelectAttribute, a2.ProductAttribute.Name);
                ...
                if (a2.AttributeControlType == AttributeControlType.ReadonlyCheckboxes)
                {
                    ...
                    if (!CommonUtil.ArraysEqual(allowedReadOnlyValueIds, selectedReadOnlyValueIds))
                    {
            ...
            foreach (var pam in attributes2)
            {
                ...
                if (pam.ValidationMinLength.HasValue)
                {
                        ...
                        pam.AttributeControlType == AttributeControlType.MultilineTextbox)
                    {
                        ...
                        {
                            warnings.Add(string.Format(InfoMsg.ShoppingCart_TextboxMinimumLength, pam.ProductAttribute.Name, pam.ValidationMinLength.Value));
                        }
                    }
                }
                ...
                if (pam.ValidationMaxLength.HasValue)
                {
                        ...
                        pam.AttributeControlType == AttributeControlType.MultilineTextbox)
                    {
                        ...
                        {
                            warnings.Add(string.Format(InfoMsg.ShoppingCart_TextboxMaximumLength, pam.ProductAttribute.Name, pam.ValidationMaxLength.Value));
                        }
                    }
                }
            }
            ...
            foreach (var attributeValue in attributeValues)
            {
                if (attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
                {
                    ...
                    if (associatedProduct != null)
                    {
                        ...
                        {
                            var attributeName = attributeValue.ProductAttributeMapping.ProductAttribute.Name;
                            var attributeValueName = attributeValue.Name;
                            warnings.Add(string.Format(
                                InfoMsg.ShoppingCart_AssociatedAttributeWarning,
            ...
            Product product, string attributesXml)
        {
            ...
            if (product.IsGiftCard)
            {
                ...
                if (String.IsNullOrEmpty(giftCardRecipientName))
                    warnings.Add(InfoMsg.ShoppingCart_RecipientNameError);

                if (product.GiftCardType == GiftCardType.Virtual)
                {
                    //validate for virtual gift cards only
                    if (String.IsNullOrEmpty(giftCardRecipientEmail) || !CommonUtil.IsValidEmail(giftCardRecipientEmail))
                        warnings.Add(InfoMsg.ShoppingCart_RecipientEmailError);
                }

                if (String.IsNullOrEmpty(giftCardSenderName))
                    warnings.Add(InfoMsg.ShoppingCart_SenderNameError);

                if (product.GiftCardType == GiftCardType.Virtual)
                {
                    //validate for virtual gift cards only
                    if (String.IsNullOrEmpty(giftCardSenderEmail) || !CommonUtil.IsValidEmail(giftCardSenderEmail))
                        warnings.Add(InfoMsg.ShoppingCart_SenderEmailError);
                }
            }
            ...
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null)
        {
            ...
            {
                warnings.Add(InfoMsg.ShoppingCart_Rental_EnterStartDate);
                return warnings;
            }
            if (!rentalEndDate.HasValue)
            {
                warnings.Add(InfoMsg.ShoppingCart_Rental_EnterEndDate);
                return warnings;
            }
            if (rentalStartDate.Value.CompareTo(rentalEndDate.Value) > 0)
            {
                warnings.Add(InfoMsg.ShoppingCart_Rental_StartDateLessEndDate);
                return warnings;
            }
            ...
            //today
            DateTime nowDtInStoreTimeZone = DateTime.Now;// _dateTimeHelper.ConvertToUserTime(DateTime.Now, TimeZoneInfo.Local, _dateTimeHelper.DefaultStoreTimeZone);
            var todayDt = new DateTime(nowDtInStoreTimeZone.Year, nowDtInStoreTimeZone.Month, nowDtInStoreTimeZone.Day);
            DateTime todayDtUtc = todayDt.ToUniversalTime();// _dateTimeHelper.ConvertToUtcTime(todayDt, _dateTimeHelper.DefaultStoreTimeZone);
            //dates are entered in store timezone (e.g. like in hotels)
            DateTime startDateUtc = rentalStartDate.Value.ToUniversalTime();// _dateTimeHelper.ConvertToUtcTime(rentalStartDate.Value, _dateTimeHelper.DefaultStoreTimeZone);
            ...
            {
                warnings.Add(InfoMsg.ShoppingCart_Rental_StartDateShouldBeFuture);
                return warnings;
            }
        ...
        /// <returns>Warnings</returns>
        public virtual IList<string> GetShoppingCartItemWarnings(User customer, ShoppingCartType shoppingCartType,
            ...
            string checkoutAttributesXml, bool validateCheckoutAttributes)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    warnings.Add(string.Format(InfoMsg.ShoppingCart_CannotLoadProduct, sci.ProductId));
                    return warnings;
                }
            ...
            if (hasStandartProducts && hasRecurringProducts)
                warnings.Add(InfoMsg.ShoppingCart_CannotMixStandardAndAutoshipProducts);
            ...
            if (hasRecurringProducts)
            {
                ...
                int totalCycles;
                string cyclesError = shoppingCart.GetRecurringCycleInfo(
            ...
            if (validateCheckoutAttributes)
            {
                ...
                foreach (var a2 in attributes2)
                {
                    if (a2.IsRequired)
                    {
                        ...
                        if (!found)
                        {
                            if (!string.IsNullOrEmpty(a2.TextPrompt))
                                warnings.Add(a2.TextPrompt);
                            else
                                warnings.Add(string.Format(InfoMsg.ShoppingCart_SelectAttribute, a2.Name));
                        }
                    }
                }
                ...
                foreach (var ca in attributes2)
                {
                    if (ca.ValidationMinLength.HasValue)
                    {
                            ...
                            ca.AttributeControlType == AttributeControlType.MultilineTextbox)
                        {
                            ...
                            {
                                warnings.Add(string.Format(InfoMsg.ShoppingCart_TextboxMinimumLength, ca.Name, ca.ValidationMinLength.Value));
                            }
                        }
                    }
                    ...
                    if (ca.ValidationMaxLength.HasValue)
                    {
                            ...
                            ca.AttributeControlType == AttributeControlType.MultilineTextbox)
                        {
                            ...
                            {
                                warnings.Add(string.Format(InfoMsg.ShoppingCart_TextboxMaximumLength, ca.Name, ca.ValidationMaxLength.Value));
                            }
                        }
                    }
                }
        ...
        /// <param name="automaticallyAddRequiredProductsIfEnabled">Automatically add required products if enabled</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> AddToCart(User customer, Product product,
            ...
            int quantity = 1, bool automaticallyAddRequiredProductsIfEnabled = true)
        {
            ...
            if (quantity <= 0)
            {
                warnings.Add(InfoMsg.ShoppingCart_QuantityShouldPositive);
                return warnings;
            }
                ...
                if (warnings.Count == 0)
                {
                    ...
                    switch (shoppingCartType)
                    {
                        case ShoppingCartType.ShoppingCart:
                            {
                                if (cart.Count >= ShoppingCartSettings.MaximumShoppingCartItems)
                                {
                                    warnings.Add(string.Format(InfoMsg.ShoppingCart_MaximumShoppingCartItems, ShoppingCartSettings.MaximumShoppingCartItems));
                                    return warnings;
                                }
                            }
                            break;
                        case ShoppingCartType.Wishlist:
                            {
                                if (cart.Count >= ShoppingCartSettings.MaximumWishlistItems)
                                {
                                    warnings.Add(string.Format(InfoMsg.ShoppingCart_MaximumWishlistItems, ShoppingCartSettings.MaximumWishlistItems));
                                    return warnings;
                                }
                            }
                            break;
        ...
        /// <param name="resetCheckoutData">A value indicating whether to reset checkout data</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> UpdateShoppingCartItem(User customer,
        ...
        public virtual void MigrateShoppingCart(User fromCustomer, User toCustomer, bool includeCouponCodes)
        {
            ...
            {
                //discount
                var discountCouponCode = fromCustomer.GetAttribute<string>(SystemCustomerAttributeNames.DiscountCouponCode,_genericAttributeService);
                ...
                //gift card
                foreach (var gcCode in fromCustomer.ParseAppliedGiftCardCouponCodes(_genericAttributeService))
                    toCustomer.ApplyGiftCardCouponCode(gcCode, _genericAttributeService);
