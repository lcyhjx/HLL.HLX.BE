...
{
    public class PictureDomainService : DomainService
    {
        #region Const
        ...
        private readonly SettingDomainService _settingService;
        ...
        public MediaSettings MediaSettings
        {
            get
            {
                ...
                {
                    _mediaSettings1 = _settingService.LoadSetting<MediaSettings>(_storeContext.CurrentStore.Id);
                }
        ...
        protected virtual void DeletePictureThumbs(Picture picture)
        {
            ...
            var thumbDirectoryPath = WebHelper.MapPath("~/content/images/thumbs");
        ...
        {
            var thumbsDirectoryPath = WebHelper.MapPath("~/content/images/thumbs");
        ...
        {
            return Path.Combine(WebHelper.MapPath("~/content/images/"), fileName);
        }
                    ...
                    if (!GeneratedThumbExists(thumbFilePath, thumbFileName))
                    {
                        using (var stream = new MemoryStream(pictureBinary))
                        {
                            ...
                            catch (ArgumentException exc)
                            {
                                ...
                                Logger.Error(string.Format("Error generating picture thumb. ID={0}", picture.Id), exc);
                            }
                            if (b == null)
                            {
        ...
        public virtual Picture GetPictureById(int pictureId)
        {
            ...
            return _pictureRepository.FirstOrDefault(pictureId);
        }
        ...
        {
            mimeType = CommonUtil.EnsureNotNull(mimeType);
            mimeType = CommonUtil.EnsureMaximumLength(mimeType, 20);

            seoFilename = CommonUtil.EnsureMaximumLength(seoFilename, 100);
