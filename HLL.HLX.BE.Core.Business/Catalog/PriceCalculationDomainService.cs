...
{
    public class PriceCalculationDomainService : DomainService
    {
        #region Fields

        //private readonly IWorkContext _workContext;
        private readonly IStoreContext _storeContext;
        private readonly DiscountDomainService _discountService;
        private readonly CategoryDomainService _categoryService;
        private readonly ManufacturerDomainService _manufacturerService;
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToCategories(Product product, User customer)
        {
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToCategories))
            {
                ...
                //{
                    ...
                    //return categoryIds;
                    var appliedToCategoryIds = categoryIds;
                //});
                ...
                if (appliedToCategoryIds.Any())
                {
                    ...
                    //    .ToList());
                    var categoryIds1 = _categoryService
                        .GetProductCategoriesByProductId(product.Id)
                        .Select(x => x.CategoryId)
                        .ToList();
                    foreach (var id in categoryIds1)
                    {
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToManufacturers(Product product, User customer)
        {
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToManufacturers))
            {
                ...
                var appliedToManufacturerIds = discount.AppliedToManufacturers.Select(x => x.Id).ToList();
        ...
        protected virtual decimal? GetMinimumTierPrice(Product product, User customer, int quantity)
        {
            ...
            out Discount appliedDiscount)
        {
            ...
            var cacheTime = CatalogSettings.CacheProductPrices ? 60 : 0;
            ...
            out Discount appliedDiscount)
        {
            ...
            else
            {
                ...
                {
                    int qty;
                    if (ShoppingCartSettings.GroupTierPricesForDistinctShoppingCartItems)
                    {
                ...
                case AttributeValueType.AssociatedToProduct:
                    {
                        ...
                        if (associatedProduct != null)
                        {
                            ...
                            adjustment = GetFinalPrice(associatedProduct, new User(), includeDiscounts: true) * value.Quantity;
                        }
                    }
                    break;
                default:
